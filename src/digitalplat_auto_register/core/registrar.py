...
class DigitalPlatRegistrar:
    ...
    def __init__(self, config: DigitalPlatConfig):
        ...
        self._email_was_auto_created = False
    ...
    async def register_account(
        ...
        try:
            ...
            self._email_was_auto_created = email is None
    ...
    def _email_session_ready(self, email: str) -> bool:
        """Return whether the current service owns a usable mailbox session."""
        service = self.email_service
        return bool(
            service
            and getattr(service, "current_email", None) == email
            and getattr(service, "auth_token", None)
            and getattr(service, "account_id", None)
        )
    ...
    async def _execute_registration_workflow(
        ...
        try:
            ...
            # A generated mailbox is tied to the EmailService instance that
            # created it. Retries recreate services, so establish a new mailbox
            # session instead of reusing only the previous address.
            create_temporary_email = not user_profile.email
            if (
                user_profile.email
                and self._email_was_auto_created
                and not self._email_session_ready(user_profile.email)
            ):
                logger.info(
                    f"Email session is unavailable for {user_profile.email}; "
                    "creating a fresh temporary email"
                )
                create_temporary_email = True

            if create_temporary_email:
    ...
    async def _wait_for_verification_email(self, email: str) -> VerificationEmailResult:
        ...
        # Pass browser page for browser-based fallback if HTTP API fails
        browser_page = None
        if self.browser_service and self.browser_service.page:
            browser_page = self.browser_service.page
